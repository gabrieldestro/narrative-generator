...
import { isGroundedTerm } from "../../domain/utils/grounding.js";
...
export class WorldService {
  ...
  public applyConditionsDelta(
    ...
    for (const entry of delta.conditions ?? []) {
      ...
      if (current.length >= 3) continue; // max 3 por char
      updated = {
        ...updated,
        characters: updated.characters.map((c) =>
  ...
  public advanceVitality(state: GameState, name: string, dir: 'worse' | 'better'): GameState {
    ...
    const idx = WorldService.VITALITY_LADDER.indexOf(current);
    const next = dir === 'worse'
      ? WorldService.VITALITY_LADDER[Math.min(idx + 1, 3)]
      : WorldService.VITALITY_LADDER[Math.max(idx - 1, 0)];
