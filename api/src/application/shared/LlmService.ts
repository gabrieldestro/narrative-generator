...
import { ArbiterService } from "../turn/ArbiterService.js";
import { MemoryService } from "../turn/MemoryService.js";
import {
...
export class LlmService {
  ...
  /** Doc 27, Fase 4: memória factual junta (`MemoryService`). */
  ...
  async arbitrateLogic(state: GameState, actions: string[], recentHistory?: string[], longTermSummary?: string): Promise<string> {
    // Delega ao `ArbiterService.arbitrateTurn` — mesmos prompts de
    // `prompts.ts`, mesmo `invokeWithRetry`, zero mudança de fluxo.
    return this.arbiterService.arbitrateTurn(state, actions, recentHistory, longTermSummary);
  }
  ...
  async summarizeMemory(longTermSummary: string | undefined, oldestTurns: string[], turn = 0): Promise<string> {
    // Fase 4 (doc 27): delega ao `MemoryService` (mesmos prompts, zero mudança).
    return this.memoryService.summarizeMemory(longTermSummary, oldestTurns, turn);
  }

  async updateWorldContext(currentContext: string, lastNarration: string, turn = 0): Promise<string> {
    // Fase 4 (doc 27): delega ao `MemoryService` (mesmos prompts, zero mudança).
    return this.memoryService.updateWorldContext(currentContext, lastNarration, turn);
  }
