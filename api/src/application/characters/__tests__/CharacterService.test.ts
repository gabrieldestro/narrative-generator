...
describe('CharacterService', () => {
  ...
  beforeEach(() => {
    ...
    service = new CharacterService(llmService, { maxScratchpadSize: 5, maxCpuRetries: 3 });
  });
  ...
  describe('reflectAndAct', () => {
    ...
    it('deve lançar erro após 3 tentativas consecutivas com JSON inválido', async () => {
      ...
      await expect(service.reflectAndAct(makeState(), makeChar())).rejects.toThrow(
        /CharacterService.reflectAndAct falhou após 3 tentativas/
      );
...
describe('CharacterService — integração com LlmService real mockado', () => {
  it('deve funcionar com retry e saída visível no output mock', async () => {
    ...
    const service = new CharacterService(llmService, { maxScratchpadSize: 5, maxCpuRetries: 3 });
