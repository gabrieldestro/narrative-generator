...
* **Fases concluídas:** CLI → API Fastify → Frontend Angular → persistência em disco (checkpoints imutáveis) → turno por steps (orquestrador) → controllers segregados por área.
...
* **Comentários no código:** só o necessário para entender o "porquê". **Nunca** referenciar documentos de implementação (sessões, fases, `§`, `doc N`, etc.) — o código deve se sustentar sozinho.
